...
def CreateMultiBoxPoseHead(net, data_layer="data", num_classes=[], num_poses=[], from_layers=[],
        use_objectness=False, normalizations=[], use_batchnorm=True,
        min_sizes=[], max_sizes=[], prior_variance = [0.1],
        aspect_ratios=[], share_location=True, share_pose=True, flip=True, clip=True,
        inter_layer_depth=0, kernel_size=1, pad=0, conf_postfix='', pose_postfix='' , loc_postfix=''):
    assert num_classes, "must provide num_classes"
    assert num_classes > 0, "num_classes must be positive number"
    
    assert num_poses, "must provide num_poses"
    assert num_poses > 0, "num_poses must be positive number"
    if normalizations:
        assert len(from_layers) == len(normalizations), "from_layers and normalizations should have same length"
    assert len(from_layers) == len(min_sizes), "from_layers and min_sizes should have same length"
    if max_sizes:
