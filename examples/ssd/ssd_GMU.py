...
class SSD:
    ...
    def run_main(self, args, data_dir):
        ...
        resume_training = args.get_opts('resume')
        # If true, Remove old model files.
        #remove_old_models = args.get_opts('remove')

        idx = args.get_opts('mod_id')

        gpus = args.get_opts('gpu')
        gpulist = gpus.split(",")
        ...
        batch_sampler = makeBatchSampler(args)
        ...
        if use_batchnorm:
            base_lr = args.get_opts('base_lr')  * 1000
        else:
            # A learning rate for batch_size = 1, num_gpus = 1.
            base_lr = args.get_opts('base_lr') 
            #base_lr = 0.00004
            #base_lr = 0.0000004
        ...
        job_name = '%d' % idx
        model_name = "VGG_GMU_{}".format(job_name)

        # Directory which stores the model .prototxt file.
        save_dir = "models/VGGNet/gmu_kitchen/{}".format(job_name)
        # Directory which stores the snapshot of models.
        snapshot_dir = "models/VGGNet/gmu_kitchen/{}".format(job_name)
        # Directory which stores the job script and log file.
        job_dir = "jobs/VGGNet/gmu_kitchen/{}".format(job_name)
        # Directory which stores the detection results.
        output_result_dir = "data/gmu_kitchen/results/{}/Main".format(job_name)
        ...
        # Stores LabelMapItem.
        label_map_file = osp.join(data_dir, 'labelmap.prototxt')
        ...
        conf_name = "mbox_conf"
        if multibox_loss_param["conf_loss_type"] == caffe_pb2.ConfLossType.Value('ConfLossType_SOFTMAX'):
        ...
        elif multibox_loss_param["conf_loss_type"] == caffe_pb2.ConfLossType.Value('ConfLossType_LOGISTIC'):
