...
void DetectionEvaluatePoseLayer<Dtype>::LayerSetUp(
...
template <typename Dtype>
void DetectionEvaluatePoseLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void DetectionEvaluatePoseLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  ...
  GetDetectionPoseResults(det_data, bottom[0]->height(), background_label_id_,
                      &all_detections);

  // Retrieve all ground truth (including difficult ones).
  map<int, LabelBBox> all_gt_bboxes;
  // RIC
  GetGroundTruthPose(gt_data, bottom[1]->height(), background_label_id_,
                 true, &all_gt_bboxes);
      ...
      for (LabelBBox::iterator iit = detections.begin();
        ...
        if (label_bboxes.find(label) == label_bboxes.end()) {
          ...
          for (int i = 0; i < bboxes.size(); ++i) {
            ...
            float overlap_max = -1;
            int jmax = -1;
            // Finding which ground truth box the predicted box overlaps with them most
...
INSTANTIATE_CLASS(DetectionEvaluatePoseLayer);
REGISTER_LAYER_CLASS(DetectionEvaluatePose);
