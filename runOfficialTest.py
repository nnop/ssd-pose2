...
def main(args):
  ...
  skip = False
  if osp.exists(osp.join('mat_eval', fiOutput, 'car.mat')):
    skip = True
  ...
  for idx, v in enumerate(val):
      ...
      if not osp.exists(imfile):
          imfile = osp.join('data/pascal3D/PASCAL/VOCdevkit/VOC2012/JPEGImages/', v + '.jpg')
          #print 'why'
  ...
  for lbl, all_val in all_out.iteritems():
      if skip:
        break
      if not osp.exists('mat_eval'):
        os.mkdir('mat_eval')
        os.mkdir(osp.join('mat_eval', fiOutput))
      if not osp.exists(osp.join('mat_eval', fiOutput)):
        os.mkdir(osp.join('mat_eval', fiOutput))
